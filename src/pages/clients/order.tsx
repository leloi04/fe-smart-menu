...
import { addCustomerToOrderAPI, createOrderAPI, getOrderByTable, getTableAPI, verifyTableTokenAPI } from "@/services/api"; // 👉 API kiểm tra token
...
const OrderPage = () => {
  ...
  const [step, setStep] = useState<"verify" | "choose" | "guestName" | "order">("verify");
  const [statusTable, setStatusTable] = useState<string>("");
  const [tableData, setTableData] = useState<{ tableId: string; tableNumber: string } | null>(null);
  ...
  const handleGuestSubmit = async () => {
    ...
    const newGuest: UserInfo = { userId: uuidv4(), name: guestName, isGuest: true };
  ...
  const handleLogin = () => {
    navigate(
      `/login?redirect=${encodeURIComponent(`/tables/${tableNumber}?token=${token}`)}`
    );
  };
  ...
  if(statusTable === "cleaning") {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gray-50">
        <p className="text-gray-600">Bàn đang được dọn...</p>
      </div>
    );
  } else {
