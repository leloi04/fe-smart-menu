 ...
 const handleQuantityChange = (id: string, delta: number) => {
  ...
  const handleToppingToggle = (itemId: string, toppingId: string) => {
    ...
    setOrders((prev) => {
      ...
      const updated = selected.includes(toppingId)
        ...
        : [...selected, toppingId];
      return { ...prev, [itemId]: { ...prev[itemId], selectedToppings: updated } };
    });
  }
  ...
  const handleSendOrder = (isAddItems: boolean) => {
    ...
    socket.emit("sendOrder", {
      ...
      tableNumber,
      isAddItems,
      statusChanged: !isAddItems ? "pending_confirmation" : undefined,
    });
  };
  ...
  return (
    ...
    {/* Body order */}
    <div className="flex flex-col items-center bg-[#fff7ed] min-h-screen pb-28 pt-8 px-4 mt-16 mb-12"> {/* add bottom padding so content not hidden by fixed bar */}
      <h1 className="text-3xl font-bold text-[#7c2d12] mb-4">🍽️ Thực đơn bàn ngày hôm nay</h1>
          ...
          return (
            <div
              ...
              className={`relative border rounded-xl p-3 shadow-md ${
                isAvailable ? "bg-[#ffedd5] hover:shadow-lg" : "bg-[#fde68a] opacity-70 cursor-not-allowed"
              }`}
            >
              <div className="flex gap-4 items-start">
                <img src={`${import.meta.env.VITE_BACKEND_URL}/images/menu/${item.image}`} alt={item.name} className="w-24 h-24 rounded-lg object-cover" />
                    ...
                    {item.variants.map((v) => (
                      <label
                        ...
                        className={`cursor-pointer px-3 py-1 border rounded-full ${
                          order.selectedVariant === v._id ? "bg-[#7c2d12] text-white" : "text-gray-700 border-gray-300"
                        }`}
                      >
                        <input type="radio" name={`variant-${item._id}`} className="hidden" onChange={() => handleVariantSelect(item._id, v._id)} />
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.toppings.map((t) => (
                      <label
                        key={t._id}
                        className={`cursor-pointer px-3 py-1 border rounded-full ${
                          order.selectedToppings.includes(t._id) ? "bg-[#78350f] text-white" : "text-gray-700 border-gray-300"
                        }`}
                      >
                        <input type="checkbox" className="hidden" onChange={() => handleToppingToggle(item._id, t._id)} />
              ...
              {isAvailable && (
                <div className="flex items-center justify-between mt-3">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => handleQuantityChange(item._id, -1)} className="bg-[#7c2d12] text-white w-8 h-8 rounded-full">
                      -
                    </button>
                    <span className="text-lg font-bold text-[#7c2d12] w-6 text-center">{order.quantity}</span>
                    <button onClick={() => handleQuantityChange(item._id, 1)} className="bg-[#7c2d12] text-white w-8 h-8 rounded-full">
                      +
                    </button>
                  </div>
        ...
        >
          ...
          >
            ...
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-[#7c2d12]">🧾 Chi tiết Order</h2>
              <button onClick={() => setShowOrderPanel(false)} className="text-gray-500 text-sm">
                Đóng
              </button>
            </div>

            {/* Món đang chế biến lần đầu */}
              <h2 className="font-semibold text-[#7c2d12] mb-2">Món đang chế biến</h2>
            <div className="mb-4">
              <h2 className="font-semibold text-[#9d5237] mb-2 ml-2">Món gọi ban đầu</h2>
              {firstOrder?.orderItems.length > 0 ? (
                firstOrder.orderItems.map((oi: any) => (
                  <div key={oi.menuItemId} className="border p-3 rounded-lg mb-2 bg-orange-50">
            ...
            <div className="mb-4">
              <h3 className="font-semibold text-[#7c2d12] mb-2">Món đã xong</h3>
              {realtimeOrder?.completedItems?.length > 0 ? (
                realtimeOrder.completedItems.map((oi: any) => (
                  <div key={oi.menuItemId} className="border p-3 rounded-lg mb-2 bg-green-100">
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="font-semibold text-[#7c2d12]">{oi.name}</p>
                        {oi.variant && <p className="text-sm text-gray-600">Size: {oi.variant.size}</p>}
          ...
          <div className="bg-amber-800 text-white p-4 flex justify-between items-center shadow-lg">
            ...
            <p className="font-bold text-xl">{(totalPriceAddItems + totalPriceFirstOrder).toLocaleString("vi-VN")} ₫</p>
          </div>
        </div>
      )}
  ...
  <div className="w-full max-w-[700px] px-4 pointer-events-auto">
    <div className="bg-gradient-to-r from-amber-800 to-amber-700 text-white p-4 rounded-xl shadow-lg">
      ...
      {/* Tổng tiền */}
      {(progressStatus === "draft" || progressStatus === "processing") && (
        <div className="flex justify-between w-full mb-3">
          <p className="font-semibold opacity-90">Tổng cộng</p>
          <p className="text-2xl font-bold tracking-wide">{totalPrice.toLocaleString("vi-VN")} ₫</p>
        </div>
      )}
      ...
      <div className="flex gap-3 w-full">
        ...
        {(progressStatus === "draft" || progressStatus === "pending_confirmation") && (
          <button
            onClick={() => progressStatus === "draft" ? handleSendOrder(false) : undefined}
          ...
          >
            {progressStatus === "draft" ? "🚀 Gửi Order" : "⏳ Đang chờ xác nhận..."}
          </button>
        )}
      </div>
    </div>
